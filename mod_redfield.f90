...
!write(1,*) t,real(su1),aimag(su1)!,su1_dot,su1
!write(2,*) t,real(su1_dot),aimag(su1_dot)!,su1_dot,su1
!write(3,*) t,real(su1_dot2),aimag(su1_dot2)!,su1_dot,su1
!write(*,*) su1,su1_dot2
!stop
l_exit=0d0
c_p=0d0
...
!write(*,*) A(i),F(i),i
!write(*,*) (iota*(E_excit(i)-2d0*l_exit(i,i,i,i)))*t/hbar
enddo
...
R_diss_t(i,j)=(kai(i,j)*A(i)*conjg(F(j)))

!write(*,*) g_p(i,i,j,j),l_exit(i,i,j,j)*t/hbar
!write(*,*) R_diss_t(i,j),i,j 
!write(*,*) kai(i,j),i,j 
!write(*,*) A(i)*conjg(F(j)),i,j 
...
!write(*,*) R_diss(1,1),R_diss(2,2)
!write(*,*) R_diss(1,2),R_diss(2,1)
...
ki(i,j)=ki(i,j)-iota*((E_excit(i)-E_excit(j))/hbar)*rho(i,j)+(R_coh(i,j))*rho(i,j)
...
!k4
call calculate_k(rho,k4)


  rho=rho+dt/6.d0*(k1+2d0*k2+2d0*k3+k4)
